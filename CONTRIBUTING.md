# Contributing to stashcore-lib

We're working hard to make *stashcore-lib* the most powerful JavaScript library for working with Stash. Our goal is to have *stashcore-lib* be a library that can be used by anyone interested in Stash, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://www.stash.org/forum (Support Forum)
...
These are some global design goals in stashcore-lib that any change must adhere.
...
When possible, stashcore-lib objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of stashcore-lib methods. For example, calling `JSON.stringify` on an stashcore-lib object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'stashcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the stashcore object.

DO:
```javascript
var stashcore = require('../');
var PublicKey = stashcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add stashpayio git@github.com:stashpayio/stashcore-lib.git
git pull --rebase stashpayio master
```
...
```
Finally go to [github.com/stashpayio/stashcore-lib](https://github.com/stashpayio/stashcore-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of stashcore-lib.

If you have any questions feel free to post them to
[github.com/stashpayio/stashcore-lib/issues](https://github.com/stashpayio/stashcore-lib/issues).
